...
for i in range(1): 
        ...
        #print(idx)
        Xs = torch.tensor(Y[:,idd[0:n]]+0.2*np.random.randn(input_dim,n)).float()
        use_gpu = False
        ...
        for epoch in range(num_epochs):
            ...
            if use_gpu == True:
                ...
                loss = loss_func(output, Xs.to(device)).to(device) +   loss_func2(output, Xs.to(device)).to(device)                                                         
            else:
                output = model()
                #print(output)
                loss = loss_func(output, Xs)+loss_func2(output, Xs)#+ 0.01*model.regularization()
            loss.backward()
            optimizer.step()
